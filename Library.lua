...
local Misc = Window:Tab({Title = "Misc", Icon = "box"}) do
    ...
    Misc:Textbox({
        ...
        Desc = "Peluru keluar dalam 1 tembakan (Max 100)",
        Placeholder = "60",
        ...
        Callback = function(value)
            if tonumber(value) then
                ...
                set_setting("Multiply", tonumber(value)) 
            end
        end
    })
    ...
    Misc:Toggle({
        ...
        Callback = function(state)
            ...
            set_setting("NoRecoilGatling", state)
        end
    })
    ...
    Misc:Button({
        ...
        Callback = function()
            local success, err = pcall(function()
                ...
                gganim._fireGun = function(self)
                    ...
                    if currentAmmo and currentAmmo <= 1 and not isReloading then
                        ...
                        -- Visual Fast Reload (Mempercepat animasi di layar)
                        if self.Model and self.Model:FindFirstChild("Character") and self.Model.Character:FindFirstChild("Humanoid") then
                            local animator = self.Model.Character.Humanoid:FindFirstChild("Animator")
                            if animator then
            ...
            if success then
                ...
                Window:Notify({Title = "ERROR", Desc = "Gagal: Pasang Gatling Gun terlebih dahulu!", Time = 4, Type = "error"})
            end
        end
    })
        ...
        Callback = function()
            ...
            if towersFolder then
                for _, tower in pairs(towersFolder:GetChildren()) do
                    ...
                    if rep and rep:GetAttribute("OwnerId") == local_player.UserId and rep:GetAttribute("Name") == "Gatling Gun" then
